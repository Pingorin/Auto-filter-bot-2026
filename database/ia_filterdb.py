# database/ia_filterdb.py (umongo and motor integration)
import re
...
class Media(Document):
    ...
    class Meta:
        # Indexing: cleaned_name par index banana (search optimization ke liye)
        # Note: Text indexing MongoDB Atlas ya manually create karna padega.
        # Simple single-field index for comparison based search.
...
def get_file_details(media):
    ...
    try:
        ...
        # file_ref bytes format mein milega (umongo BinaryField ke liye perfect)
...
async def save_file(media, caption):
    ...
    try:
        # Document ko save karna (umongo automatically unique check handle karega)
...
async def get_search_results(query: str, file_type: str = None, max_results: int = 10, offset: int = 0):
    ...
    # Check if caption filter is configured
    ...
    # umongo Document class use karke find karna (jo motor ko use karta hai)
    cursor = Media.find(query_filter).skip(offset).limit(max_results)
    
    # .to_list() method motor se aata hai, umongo find se cursor milta hai
    results = await cursor.to_list(length=max_results)
    
    # Results umongo Documents ke roop mein milenge.
    return results
